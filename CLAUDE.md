# CLAUDE.md — gss-beacon

Project rules for this repo — for both AI agents and human contributors. See
[README.md](README.md) for what the beacon is and how it's built.
...
- **Types:** `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`,
  `ci`, `chore`, `revert`.
- **Scope** is optional but encouraged — e.g. `beacon`, `mute`, `schema`, `loader`,
  `deploy`.
...
`schema.sql` is the D1 table definition. When you change it, apply it to the remote
DB (`npm run schema`) as part of the same change — a deploy without the migration
will break inserts. The `excluded_ips` table (owner opt-out) must exist before the
beacon references it.
...
`README.md` and `CHANGELOG.md` in the same commit as any behavior change.

## Deploy

Manual `wrangler` deploy — the token comes from a local, **gitignored** file (never
commit it). See the [Deploy](README.md#deploy) section of the README.
